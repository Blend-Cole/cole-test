...
// Define the isInUSA function
const isInUSA = (lat, lon) => {
  return lat >= 24.396308 && lat <= 49.384358 && lon >= -125.0 && lon <= -66.93457;
};

// Updates the cart attributes
const updateCartAttributes = (value) => fetch("/cart/update.js", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ attributes: { 'international_visitor': value } })
}).then(response => response.json())
  .then(data => console.log("Cart attributes updated:", data))
  .catch(error => console.error("Error updating cart attributes:", error));

// Check if the user is an international visitor
if (!localStorage.getItem("location_determined")) {
  navigator.geolocation.getCurrentPosition(
    ({ coords: { latitude, longitude } }) => {
      console.log(`Latitude: ${latitude}, Longitude: ${longitude}`);
      if (!isInUSA(latitude, longitude)) {
        updateCartAttributes('true');
        console.log('International Visitor')
      }
      else {
        updateCartAttributes('false');
        console.log('United States Visitor')
      }
      localStorage.setItem("location_determined", "true");
    },
    (error) => {
